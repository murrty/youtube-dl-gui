...
namespace youtube_dl_gui {
    static class Program {
        ...
        public static volatile Ini ini;
        ...
        static void Main(string[] args) {
            ...
            Config.ProgramConfig.Load(ConfigType.Initialization);
            ...
            if (IsDebug) {
                ...
                if (Config.ProgramConfig.Initialization.firstTime) {
                    if (MessageBox.Show("youtube-dl-gui is a visual extension to youtube-dl and is not affiliated with the developers of youtube-dl in any way.\n\nThis program (and I) does not condone piracy or illegally downloading of any video you do not own the rights to or is not in public domain.\n\nAny help regarding any problems when downloading anything illegal (in my jurisdiction) will be ignored. This message will not appear again.\n\nHave you read the above?", "youtube-dl-gui", MessageBoxButtons.YesNo) == DialogResult.Yes) {
                        Config.ProgramConfig.Initialization.firstTime = false;
                        ...
                        if (MessageBox.Show("Downloads are saved to your downloads folder by default, would you like to specify a different location now?\n(You can change this in the settings at any time)", "youtube-dl-gui", MessageBoxButtons.YesNo) == DialogResult.Yes) {
                            using (FolderBrowserDialog fbd = new FolderBrowserDialog()) {
                                ...
                                if (fbd.ShowDialog() == DialogResult.OK) {
                                    Config.ProgramConfig.Downloads.downloadPath = fbd.SelectedPath;
                                }
                                else {
                                    Config.ProgramConfig.Downloads.downloadPath = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile) + "\\Downloads";
                                }
        ...
        static void LoadClasses() {
            Config.ProgramConfig.Load(ConfigType.All);

            verif.RefreshLocation();

            if (Config.ProgramConfig.Initialization.LanguageFile != string.Empty) {
                if (System.IO.File.Exists(Environment.CurrentDirectory + "\\lang\\" + Config.ProgramConfig.Initialization.LanguageFile + ".ini")) {
                    lang.LoadLanguage(Environment.CurrentDirectory + "\\lang\\" + Config.ProgramConfig.Initialization.LanguageFile + ".ini");
                }
            }
            else {
                ...
                switch (args[1]) {
                    ...
                    case "-audio":
                        ...
                        if (Config.ProgramConfig.Downloads.AudioDownloadAsVBR) {
                            NewInfo.AudioVBRQuality = (AudioVBRQualityType)Config.ProgramConfig.Saved.audioQuality;
                        }
                        else {
                            NewInfo.AudioCBRQuality = (AudioCBRQualityType)Config.ProgramConfig.Saved.audioQuality;
                        }
                        Downloader.ShowDialog();
                        break;
                }
